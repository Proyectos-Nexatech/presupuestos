
import { createClient } from '@supabase/supabase-js';
const supabase = createClient('https://ymmkvryfinvqewodmeuw.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InltbWt2cnlmaW52cWV3b2RtZXV3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI1NjMyODEsImV4cCI6MjA4ODEzOTI4MX0.xmA-0mdvKwg_zbPxoospUIi_Se8JGPYlE0aqURshWMI');
async function run() {
    const { data: matrix } = await supabase.from('matriz_costos_calculados').select('categoria, turno_id, concepto_id, valor_final').limit(5);
    console.log('Sample Matrix:', JSON.stringify(matrix));
    const { data: shifts } = await supabase.from('configuracion_turnos').select('id, nombre');
    console.log('Shifts:', JSON.stringify(shifts));
    const { data: concepts } = await supabase.from('maestro_conceptos').select('id, codigo').in('codigo', ['AA', 'AC']);
    console.log('Concepts:', JSON.stringify(concepts));
} run().catch(console.error);
