...
async function run() {
    const { data, error } = await supabase.from('insumos').select('*').ilike('descripcion', '%TRANSPORTE%').limit(5);
    if (error) {
        console.error(error);
    } else {
        console.log('Transporte rows details:', JSON.stringify(data, null, 2));
    }
}
run();
