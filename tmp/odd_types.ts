...
async function run() {
    const { data: odd, error } = await supabase.from('insumos').select('tipo').not('tipo', 'in', '("EQUIPOS", "MATERIALES")');
    if (error) {
        console.error(error);
    } else {
        const unique = [...new Set(odd.map(i => `"${i.tipo}"`))];
        console.log('Other types:', unique);
    }
}
run();
