...
async function run() {
    const { data: q1, error } = await supabase.from('insumos').select('*').eq('tipo', 'TRANSPORTE');
    const { data: q2, error: e2 } = await supabase.from('insumos').select('*').eq('tipo', 'TRASNPORTE');

    console.log('TRANSPORTE rows:', JSON.stringify(q1, null, 2));
    console.log('TRASNPORTE rows:', JSON.stringify(q2, null, 2));
}
run();
